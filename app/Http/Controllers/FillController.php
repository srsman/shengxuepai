...
class FillController extends Controller
{
    ...
    {
        return view('function.my_volunteer');
    }
    ...
    public function volunteerList(Request $request)
    {
        $uid = 1;
        //$uid = Session::get('user_id');
        $res = VolunteerModel::select('volunteer_id', 'table_name', 'batch', 'classify', 'type', 'created_at', 'updated_at')
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getVolunteer(Request $request, $vid)
    {
        $v = VolunteerModel::select('volunteer_arg')->where('volunteer_id', $vid)
            ->first();

        $res = json_decode($v->volunteer_arg);
